	...
	return (
		<div className="flex flex-col items-center gap-4">
			...
			{user && isUserAddingItem && (
				<form className="gap-2 grid w-full" id="itemCreator" style={{ gridTemplateColumns: `repeat(${columns.length - 1}, 1fr)` }} onSubmit={handleSubmit(onSubmit)}>
					{columns.slice(0, columns.length - 1).map((col) => {
						return (
							<Input
								key={col.key}
								className="w-full"
								placeholder={inventory[col.label]}
								type={getInputType(col.label)}
								{...register(col.label.replace("Name", ""), {
									required: !col.label.includes("Bool"),
								})}
							/>
						);
					})}
			...
			{user && isUserAddingItem && (
				<div className="flex gap-2">
					<Button color="primary" form="itemCreator" type="submit">
						{t("submit")}
					</Button>
