...
export default function CreateInventory() {
	...
	const { register, handleSubmit, reset } = useForm<InventoryInput>();
	...
	return (
		<>
			...
			{error && <Alert description={error} title={"Error"} className="absolute bottom-5 right-5 w-80 bg-danger-50!" color="danger" />}
			{success && <Alert description={success} title={"Success"} className="absolute bottom-5 right-5 w-80 bg-success-50!" color="success" />}
		</>
	);
}
